...
import com.device.deviceinfosdk.entity.SimInfo
...
object SmsUtil {
    ...
    fun getIMEI(): String? {
        val tm = ContextUtil.getAppContext().getSystemService(Context.TELEPHONY_SERVICE) as TelephonyManager
        try {
            return tm?.deviceId
     ...
     */
    private fun getReflexSimInfo(index: Int): SimInfo? {
        val simInfo = SimInfo()
    ...
    private fun getReflexMethodWithId(predictedMethodName: String, id: String): String? {
        ...
        val telephony = ContextUtil.getAppContext().getSystemService(Context.TELEPHONY_SERVICE) as TelephonyManager
        try {
